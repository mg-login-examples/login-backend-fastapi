...
class AccessTokenFileStore(AccessTokenStore):
    def __init__(self, file_name: str):
        self.FILE_PATH = os.path.join(
            "stores", "access_tokens_store", "access_token_files", file_name)
        self.USER_ID_TO_ACCESS_TOKENS: dict[int, list[str]] = {}
        if os.path.isfile(self.FILE_PATH):
            with open(self.FILE_PATH, 'r') as file:
                for line in file:
                    if line.strip("\n"):
                        user_id_str, access_token = line.strip("\n").split(" ")
                        user_id = int(user_id_str)
                        if user_id in self.USER_ID_TO_ACCESS_TOKENS:
                            self.USER_ID_TO_ACCESS_TOKENS[user_id].append(
                                access_token)
                        else:
        ...
        if (await super().check_if_access_token_is_valid(access_token)):
            ...
            if token_user_id in self.USER_ID_TO_ACCESS_TOKENS:
                for user_access_token in self.USER_ID_TO_ACCESS_TOKENS[token_user_id]:
                    if check_access_tokens_are_equal(
    ...
    async def remove_access_token(self, user_id: int, access_token: str):
        if user_id in self.USER_ID_TO_ACCESS_TOKENS:
            ...
            self.USER_ID_TO_ACCESS_TOKENS[user_id] = [
                token for token in user_tokens if token != access_token]
            with open(self.FILE_PATH, 'r') as file:
