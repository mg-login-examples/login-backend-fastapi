...
def get_user_quotes(test_client: requests.Session, user_id: int, skip=0, limit=10) -> List[QuoteDeep]:
    response = test_client.get(f"/api/users/{user_id}/quotes/?skip={skip}&limit={limit}")
    assert response.status_code == 200
    assert isinstance(response.json(), List)
    quotes = [QuoteDeep(**quote_json) for quote_json in response.json()]
    return quotes
