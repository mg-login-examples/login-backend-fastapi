...
def test_get_user_fails_when_getting_different_user_than_logged_in(
    ...
    response = test_client_logged_in.get(f"/api/users/{created_user_2_by_admin.id}/")
    assert response.status_code == 403
    asserts.assert_response_error_resource_not_accessible(response)

# Test that a user can be created
@patch("fastapi.BackgroundTasks.add_task")
def test_create_user(mock_add_task: MagicMock, test_client: requests.Session, app_db_manager: SQLAlchemyDBManager):
