...
def create_user(test_client: requests.Session,
                user_to_create: UserCreate) -> UserDeep:
    response = test_client.post(
        "/api/admin/resource/users/", json=user_to_create.model_dump())
    assert response.status_code == 200
    return UserDeep(**response.json())
...
def put_user(test_client: requests.Session, user_to_edit: UserDeep):
    response = test_client.put(
        f"/api/admin/resource/users/{user_to_edit.id}/", json=user_to_edit.model_dump())
...
def get_users(test_client: requests.Session, skip=0,
              limit=10) -> list[UserDeep]:
    response = test_client.get(
        f"/api/admin/resource/users/?skip={skip}&limit={limit}")
    assert response.status_code == 200
