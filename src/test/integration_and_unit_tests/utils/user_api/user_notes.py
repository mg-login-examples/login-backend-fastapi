...
def get_user_user_notes(test_client: requests.Session,
                        user_id: int, skip=0, limit=10) -> list[UserNote]:
    response = test_client.get(
        f"/api/users/{user_id}/user-notes/?skip={skip}&limit={limit}")
    assert response.status_code == 200
...
def edit_user_note(test_client: requests.Session, user_note_to_edit: UserNote):
    response = test_client.put(
        f"/api/user-notes/{user_note_to_edit.id}/", json=jsonable_encoder(user_note_to_edit))
