...
def test_get_quote(test_client: requests.Session,
...
def test_create_quote(test_client: requests.Session,
...
def test_get_quotes(test_client: requests.Session,
                    created_n_quotes_by_admin: list[QuoteDeep]):
...
def test_put_quote(test_client_admin_logged_in: requests.Session,
                   created_n_users_by_admin: list[UserDeep], created_quote_by_admin: QuoteDeep):
    new_text = text.quote()
    ...
    quotes_admin_api.put_quote(
        test_client_admin_logged_in, created_quote_by_admin)
    quote = quotes_admin_api.get_quote(
        test_client_admin_logged_in, created_quote_by_admin.id)
...
def test_delete_quote(test_client_admin_logged_in: requests.Session,
                      created_quote_by_admin: QuoteDeep):
    quotes_admin_api.delete_quote(
        test_client_admin_logged_in, created_quote_by_admin.id)
