...
from test.integration_and_unit_tests.utils.user_api import email_verifications as email_verifications_api
...
def test_resend_email_unverified_user(mock_add_task: MagicMock, logged_in_unverified_user: User,
    ...
    db_email_verifications = crud_base.get_resource_items_by_attribute(
        db_session_1, UserEmailVerificationModel, UserEmailVerificationModel.user_id, logged_in_unverified_user.id)
    ...
    db_email_verifications = crud_base.get_resource_items_by_attribute(
        db_session_2, UserEmailVerificationModel, UserEmailVerificationModel.user_id, logged_in_unverified_user.id)
...
@patch("fastapi.BackgroundTasks.add_task")
def test_resend_email_verified_user(mock_add_task: MagicMock, test_client_logged_in: requests.Session,
                                    logged_in_user: User, app_db_manager: SQLAlchemyDBManager):
    # assert user is verified
    assert logged_in_user.is_verified == True
    # assert no email verifications record exist for user in db
    db_session_1 = next(app_db_manager.db_session())
    db_email_verifications = crud_base.get_resource_items_by_attribute(
        db_session_1, UserEmailVerificationModel, UserEmailVerificationModel.user_id, logged_in_user.id)
    assert len(db_email_verifications) == 0
    ...
    db_session_2 = next(app_db_manager.db_session())
    db_email_verifications = crud_base.get_resource_items_by_attribute(
        db_session_2, UserEmailVerificationModel, UserEmailVerificationModel.user_id, logged_in_user.id)
    assert len(db_email_verifications) == 0


@patch("fastapi.BackgroundTasks.add_task")
def test_verify_email(mock_add_task: MagicMock, logged_in_unverified_user: User,
                      test_client: requests.Session, app_db_manager: SQLAlchemyDBManager):
    ...
    db_email_verification = crud_base.get_resource_item_by_attribute(
        db_session, UserEmailVerificationModel, UserEmailVerificationModel.user_id, logged_in_unverified_user.id)
