...
def test_create_user_note(test_client_logged_in: requests.Session, logged_in_user: User):
    user_note_to_create = generate_random_user_note_to_create(logged_in_user.id)
    created_user_note = user_notes_api.create_user_note(test_client_logged_in, user_note_to_create)
...
# Test that a user note cannot be created for a user different than who is logged in
...
def test_get_user_notes_of_user_fails_when_getting_user_notes_of_different_user_than_logged_in(
    ...
    assert logged_in_user.id != created_user_2_by_admin.id
    response = test_client_logged_in.get(f"/api/users/{created_user_2_by_admin.id}/user-notes/")
    assert response.status_code == 403
    asserts.assert_response_error_resource_not_accessible(response)

# Test that a user note's title & text can be edited
def test_edit_user_note_title_and_text(test_client_logged_in: requests.Session, created_user_note: UserNote):
    ...
    edited_user_note = user_notes_api.get_user_note(test_client_logged_in, str(created_user_note.id))
    assert edited_user_note.title == "note title changed"
    assert edited_user_note.text == "note text changed"

# Test that when calling endpoint to edit a user note's title and text, changes in other fields are ignored
def test_edit_user_note_title_and_text_ignores_other_user_notes_field_changes(test_client_logged_in: requests.Session, created_user_note: UserNote):
    # Edit user note text
    ...
    user_notes_api.edit_user_note(test_client_logged_in, created_user_note)
    # Get user note by id
    edited_user_note = user_notes_admin_api.get_user_note(test_client_logged_in, str(created_user_note.id))
...
def test_edit_user_note_fails_when_updating_user_note_of_different_user_than_logged_in(
    ...
    assert logged_in_user.id != created_user_2_by_admin.id
    user_note_by_user_2_to_create = generate_random_user_note_to_create(created_user_2_by_admin.id)
    user_note_by_user_2 = user_notes_admin_api.create_user_note(test_client_admin_logged_in, user_note_by_user_2_to_create)
    assert user_note_by_user_2.text != "user note text changed"
    user_note_by_user_2.text = "user note text changed"
    response = test_client_logged_in.put(f"/api/user-notes/{user_note_by_user_2.id}/", json=jsonable_encoder(user_note_by_user_2))
    assert response.status_code == 403
    asserts.assert_response_error_resource_not_accessible(response)

# Test that a user note can be deleted by id
def test_delete_user_note(test_client_logged_in: requests.Session, created_user_note: UserNote):
    user_notes_api.delete_user_note(test_client_logged_in, str(created_user_note.id))
    user_notes_admin_api.get_user_note_expect_not_found(test_client_logged_in, str(created_user_note.id))
...
def test_delete_user_note_fails_when_deleting_user_note_of_different_user_than_logged_in(
    ...
    user_note_by_user_2_to_create = generate_random_user_note_to_create(created_user_2_by_admin.id)
    user_note_by_user_2 = user_notes_admin_api.create_user_note(test_client_admin_logged_in, user_note_by_user_2_to_create)
    response = test_client_logged_in.delete(f"/api/user-notes/{user_note_by_user_2.id}/")
    assert response.status_code == 403
    asserts.assert_response_error_resource_not_accessible(response)
