...
def test_user_delete_cascade_email_verifications(
    ...
    email_verification = UserEmailVerificationBase(
        user_id=user.id, verification_code=132213, expires_at=datetime.now())
    email_verification_db = crud_base.create_resource_item(
        db_session, UserEmailVerificationModel, email_verification)
    email_verification_id = email_verification_db.id
    # given a user with email verifications
    user_db = crud_base.get_resource_item(db_session, UserModel, user.id)
    user = UserDeep.model_validate(user_db)
    email_verification_db = crud_base.get_resource_item(
        db_session, UserEmailVerificationModel, email_verification_id)
...
def test_user_delete_cascade_user_sessions(
        app_db_manager: SQLAlchemyDBManager):
    db_session = next(app_db_manager.db_session())
    # create user
    user_login = generate_random_user_to_create()
    ...
    user_session_create = UserSessionCreate(
        user_id=user.id, token='132213', expires_at=datetime.now())
    user_session_db = crud_base.create_resource_item(
        db_session, UserSessionModel, user_session_create)
    user_session_id = user_session_db.id
    # given a user with user sessions
    user_db = crud_base.get_resource_item(db_session, UserModel, user.id)
    user = UserDeep.model_validate(user_db)
    user_session_db = crud_base.get_resource_item(
        db_session, UserSessionModel, user_session_id)
...
def test_user_delete_cascade_password_tokens(
        app_db_manager: SQLAlchemyDBManager):
    db_session = next(app_db_manager.db_session())
    # create user
    user_login = generate_random_user_to_create()
    ...
    password_token = UserPasswordResetTokenBase(
        user_id=user.id, token='132213', expires_at=datetime.now(), is_active=False)
    password_token_db = crud_base.create_resource_item(
        db_session, UserPasswordResetTokenModel, password_token)
    password_token_id = password_token_db.id
    # given a user with password tokens
    user_db = crud_base.get_resource_item(db_session, UserModel, user.id)
    user = UserDeep.model_validate(user_db)
    password_token_db = crud_base.get_resource_item(
        db_session, UserPasswordResetTokenModel, password_token_id)
...
def test_user_delete_cascade_quote_likes(app_db_manager: SQLAlchemyDBManager):
    ...
    crud_base.create_resource_item(
        db_session, UserQuoteLikeModel, user_quote_like)
    ...
    user_quote_like = crud_base.get_resource_item_by_attribute(
        db_session, UserQuoteLikeModel, UserQuoteLikeModel.user_id, user.id)
