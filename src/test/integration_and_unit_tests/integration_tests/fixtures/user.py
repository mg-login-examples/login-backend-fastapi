...
def created_unverified_user_by_admin(
        test_client_admin_logged_in: requests.Session, user_login: UserCreate) -> UserDeep:
...
@pytest.fixture
def created_user_by_admin(test_client_admin_logged_in: requests.Session,
                          created_unverified_user_by_admin: UserDeep) -> UserDeep:
    logger.debug("Create fixture created_user_by_admin")
    created_unverified_user_by_admin.is_verified = True
    users_admin_api.put_user(test_client_admin_logged_in,
                             created_unverified_user_by_admin)
    return created_unverified_user_by_admin


@pytest.fixture
def login_response(test_client: requests.Session, user_login: UserCreate,
                   created_user_by_admin: User) -> LoginResponse:
    logger.debug("Create fixture login_response")
    return authentication_api.login(test_client, user_login)


@pytest.fixture
def logged_in_unverified_user(test_client: requests.Session,
                              user_login: UserCreate, created_unverified_user_by_admin: User) -> User:
...
def created_n_users_by_admin(
        test_client_admin_logged_in: requests.Session, n_users: int = 5) -> list[UserDeep]:
...
def created_user_2_by_admin(
        test_client_admin_logged_in: requests.Session, user_2_login: UserCreate) -> UserDeep:
    logger.debug("Create fixture created_user_2_by_admin")
    user_2 = users_admin_api.create_user(
        test_client_admin_logged_in, user_2_login)
