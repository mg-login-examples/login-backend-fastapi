...
async def async_created_user_by_admin(
    async_test_client_admin_logged_in: AsyncClient, user_login: UserCreate
) -> UserDeep:
    logger.debug("Create fixture async_created_user_by_admin")
    response = await async_test_client_admin_logged_in.post(
        "/api/admin/resource/users/", json=user_login.model_dump()
    )
...
@pytest.fixture
async def async_login_response(
    async_test_client: AsyncClient,
    user_login: UserCreate,
    async_created_user_by_admin: User,
) -> LoginResponse:
