...
class PageSelectorHelper:
    def __init__(self, page: Page, selector: str,
                 selector_desc: str, user_name: str = "Anonymous"):
        self.page = page
        self.selector = selector
        self.selector_desc = selector_desc
        self.user_name = user_name

    def click(self, position: Position | None = None,
              delay: int | None = None, timeout: int | None = None):
    ...
    def hover(self, position: Position | None = None,
        ...
        def step_wrapper():
            logger.info(step_desc)
            self.page.locator(self.selector).hover(
                position=position, timeout=timeout)
        step_wrapper()

    def fill(self, text: str, timeout: int | None = None):
        step_desc = f"User '{self.user_name}' fill '{
            text}' on '{self.selector_desc}'"
    ...
    def type(self, text: str, delay: int | None = None,
             timeout: int | None = None):
        step_desc = f"User '{self.user_name}' type '{
            text}' on '{self.selector_desc}'"

        @allure.step(step_desc)
        def step_wrapper():
            logger.info(step_desc)
            self.page.locator(self.selector).type(
    ...
    def expect_to_be_visible(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to be visible"

        @allure.step(step_desc)
        def step_wrapper():
            logger.info(step_desc)
            expect(self.page.locator(self.selector)
    ...
    def expect_not_to_be_visible(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' not to be visible"
    ...
    def expect_to_be_enabled(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to be enabled"
    ...
    def check_if_disabled(self, timeout: int | None = None):
        ...
        def step_wrapper():
            ...
            return self.page.locator(
                self.selector).is_disabled(timeout=timeout)
        return step_wrapper()

    def expect_to_be_disabled(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to be disabled"

        @allure.step(step_desc)
        def step_wrapper():
            logger.info(step_desc)
            expect(self.page.locator(self.selector)
                   ).to_be_disabled(timeout=timeout)
        step_wrapper()
    ...
    def expect_to_be_checked(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to be checked"

        @allure.step(step_desc)
        def step_wrapper():
            logger.info(step_desc)
            expect(self.page.locator(self.selector)
                   ).to_be_checked(timeout=timeout)
        step_wrapper()

    def expect_not_to_be_checked(self, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' not to be checked"
    ...
    def expect_to_have_text(self, text: str, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to have text '{text}'"
    ...
    def expect_to_contain_text(self, text: str, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to contain text '{text}'"
    ...
    def expect_to_have_value(self, text: str, timeout=None):
        step_desc = f"User '{self.user_name}' expect '{
            self.selector_desc}' to have value '{text}'"

        @allure.step(step_desc)
        def step_wrapper():
            logger.info(step_desc)
            expect(self.page.locator(self.selector)
                   ).to_have_value(text, timeout=timeout)
        step_wrapper()
