...
class UserResourceTasks:
    ...
    def create_basic_user(self, user_email: str, user_password: str):
        ...
        def step_wrapper():
            ...
            self.user.on_user_create_dialog().user_email_input.expect_to_have_value(user_email)
            self.user.on_user_create_dialog().user_password_input.fill(user_password)
            self.user.on_user_create_dialog().user_password_input.expect_to_have_value(user_password)
            ...
            self.user.on_users_list_view().user_with_email(user_email).expect_to_be_visible()
        step_wrapper()
