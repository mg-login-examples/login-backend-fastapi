...
def enhance_resource_schemas(
    ...
    # update schemas
    update_schema_readonly_fields = [field for field in resource_configurations.readonly_fields if (
        field in update_resource_schema_dict["properties"].keys())]
    ...
    update_schema_designation_fields = [
        field for field in resource_configurations.designation_fields if field in update_resource_schema_dict["properties"].keys()]
    ...
    create_schema_readonly_fields = [
        field for field in resource_configurations.readonly_fields if field in create_resource_schema_dict["properties"].keys()]
    ...
    create_schema_designation_fields = [
        field for field in resource_configurations.designation_fields if field in create_resource_schema_dict["properties"].keys()]
    ...
    for resource_schema_property in resource_schema_dict["properties"]:
        ...
        if (("$ref" in schema_property) or (
            ...
            for one_of_all_resource_configurations in all_resources_configurations:
                if (
                    issubclass(resource_schema_property_type,
