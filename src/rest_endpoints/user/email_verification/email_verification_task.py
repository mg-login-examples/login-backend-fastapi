...
from data.schemas.user_email_verifications.userEmailVerificationBase import (
    UserEmailVerificationBase as UserEmailVerificationSchema,
)
...
def create_verification_code_and_send_email(
    ...
    user_email_verification = UserEmailVerificationSchema(
        verification_code=verification_code, user_id=user.id, expires_at=expires_at
    )
    crud_base.create_resource_item(
        sql_db_session, UserEmailVerificationModel, user_email_verification
    )
