...
def generate_endpoint(
    ...
    @router.get("/{user_id}/sessions/", response_model=list[UserSessionSchema], dependencies=[
                restrict_endpoint_to_own_resources_param_user_id])
    def get_user_quotes(
        user_id: int,
        skip: int = 0,
        limit: int = 100,
        ...
        userSessions = crud_base.get_resource_items_by_attribute(
            sql_db_session, UserSessionModel, UserSessionModel.user_id, user_id, skip=skip, limit=limit)
        return userSessions
