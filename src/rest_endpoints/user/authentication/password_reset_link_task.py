...
def create_password_reset_link_and_send_email(
        background_tasks: BackgroundTasks, sql_db_session: Session, user: User, app_base_url: str):
    ...
    link = f"{app_base_url}password-reset?email={user.email}&token={
        reset_password_token}&expires_at={expires_at.timestamp()}"
