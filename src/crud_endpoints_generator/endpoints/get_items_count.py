...
def generate_sql_endpoint(
    ...
    def get_resource_items_count(
            request: Request, sql_db_session: Session = sql_db_session_as_dependency) -> int:
        ...
        return sql_crud_base.get_resource_items_filtered_by_like_multiple_attributes_count(
            sql_db_session, ResourceModel, ResourceModel_attributes, attribute_filters)
...
def generate_mongo_endpoint(
    ...
    @router.get("/count/", response_model=int, dependencies=dependencies)
    def get_resource_items_count(
            mongo_db: Database = mongo_db_as_dependency) -> int:
        total_items_count = mongo_crud_base.get_resource_items_count(
            mongo_db, db_table)
