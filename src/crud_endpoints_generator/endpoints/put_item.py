...
def generate_sql_endpoint(
    ...
    def update_item(item_id: int, item: ResourceSchema,  # type: ignore
                    sql_db_session: Session = sql_db_session_as_dependency) -> Response:
...
def generate_mongo_endpoint(
    ...
    @router.put("/{item_id}/", status_code=status.HTTP_204_NO_CONTENT,
                dependencies=dependencies)
    def update_item(item_id: str, item: ResourceSchema,  # type: ignore
                    mongo_db: Database = mongo_db_as_dependency) -> Response:
        if not hasattr(item, 'id'):
            raise Exception(f'ResourceSchema "{ResourceSchema}" has no id')
