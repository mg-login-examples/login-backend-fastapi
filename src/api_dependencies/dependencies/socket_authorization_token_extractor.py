...
def get_socket_authorization_token_as_fastapi_dependency():
    def get_authorization_token(webSocket: WebSocket):
        header_authorization: str | None = webSocket.headers.get(
            "Authorization")
        cookie_authorization: str | None = webSocket.cookies.get(
            "Authorization")
        ...
        if not authorization:
            logger.error(f'Improper authorization error. Endpoint accessed: {webSocket.url}. Header Authorization: {
